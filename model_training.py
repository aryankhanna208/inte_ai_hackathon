...
model = PredictionModel(topology=topology)
...
topology.add(input=x, name="input")
topology.add(y_pred_1, name="output")
...
def normalize_images(images):
    # Reshape images to (num_samples, -1) to flatten them
    flattened_images = images.reshape(images.shape[0], -1)
    
    # Create a normalization object
    normalizer = normalization_zscore()
    
    # Compute mean and standard deviation
    normalizer.computeBatchStatistics(flattened_images)
    
    # Normalize the images
    normalized_images = normalizer.transform(flattened_images)
...
x_batch_train = normalize_images(x_batch_train)

# Loss Funstion usage
loss = tf.losses.categorical_crossentropy(y, y_pred_1)   # define the loss function
...
predictions = model.compute([input_data], engine=engine)
...
model.save(s,'./model/intel_model.h5')
